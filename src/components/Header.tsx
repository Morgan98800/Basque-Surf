    ...
    <header className="sticky top-0 z-30 bg-[#070b12]/75 backdrop-blur-2xl border-b border-white/[0.08] shadow-[0_4px_30px_rgba(0,0,0,0.6)] transition-all">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-13 py-2 gap-3">
          ...
          {tideData && (
            <div className="flex items-center liquid-glass-pill rounded-full px-3 py-1 text-white shadow-sm transition active:scale-95">
              {/* Hauteur + flèche */}
              ...
              <div className="flex items-center space-x-1.5 text-[10px] sm:text-xs text-white/60">
                <span>BM <strong className="text-white font-mono">{tideData.nextLow.time.replace(':','h')}</strong></span>
                <span className="text-white/20">•</span>
                <span>PM <strong className="text-white font-mono">{tideData.nextHigh.time.replace(':','h')}</strong></span>
              </div>
            </div>
          )}
